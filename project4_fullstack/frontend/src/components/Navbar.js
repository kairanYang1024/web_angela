...
const Actions = ({auth, customStyle, dispatch}) => {
    return (
        ...
        {auth ? (
            ...
            onClick={ () => {
              ...
              dispatch({
                type: "LOGOUT",
              }); //it's being handled here to just remove localStorage item and cascade to context API
            }}
            >
            Logout
            </button>
    ...
    ); //if auth'd, display Logout, if not, display Login and Sign Up
}

const Navbar = ({auth}) => {
    //useContext(Context) must be wrapped inside the <ContextProvider> flag
